...
(function () {
  ...
  // True date/datetime columns arrive as strings, so the typeof===number guard
  // already skips them; this only excludes numeric year/id-style columns by name.
  // HEAD (last) word match only, so a measure like "Order Value" / "ID Count"
  // still groups while "Order ID" / "Year" stay raw. Mirrors dashboard.js /
  // plot_utils._is_identifier_label.
